...
int main ( void )
{
    ...
    printfNexys("select data to display: 1. X-axis, 2.Y-axis, 3.Z-axis\n");
    char c=0;
    c=uart_inbyte();
    unsigned char recv=0;
    printfNexys("select %c ok!\n",c);
    while(1)
    {
        switch (c) {
        case '1':
            recv=read_ADXL(0x08);
            printfNexys("X-axis is %d\n",recv);
            print_digit(recv);
            break;
        case '2':
            recv=read_ADXL(0x09);
            printfNexys("Y-axis is %d\n",recv);
            print_digit(recv);
            break;
        case '3':
            recv=read_ADXL(0x0A);
            printfNexys("Z-axis is %d\n",recv);
            print_digit(recv);
            break;
        default:
            recv=read_ADXL(0x08);
